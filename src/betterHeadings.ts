...
import { produce } from "./immerCompat";
...
export const betterHeadingSettingsFacet: Facet<
  BetterHeadingSettings,
  BetterHeadingSettings
> = Facet.define<BetterHeadingSettings, BetterHeadingSettings>({
...
const regenerateState = (doc: Text, state: EditorState): BetterHeading[] => {
  const settings: BetterHeadingSettings = state.facet(
  ...
  const headings: RowMetaData[] = [...doc.iterLines()].reduce<RowMetaData[]>(
    (accumulator, lineText, index) => {
      const codeBlockFound: RegExpMatchArray | null =
        lineText.match(CODE_BLOCK_REGEX);
      ...
      if (
        !settings.startWithHeadingLevel1 &&
        accumulator.length === 0 &&
        found.groups.mdHeading === "#"
      ) {
        return accumulator;
      }
  ...
  const mdHeadings: string[] = headings
    .filter((h) => h.inCodeBlock === false)
    .map((h) => h.heading.mdHeading);
...
export const generateChanges = (state: EditorState): ChangeSpec[] => {
  const headings: BetterHeading[] =
    state.field<BetterHeading[]>(betterHeadingState);
  return headings.reduce(
    (accumulator: ChangeSpec[], heading: BetterHeading) => {
      const currentText: string = state.doc.sliceString(
        heading.from,
        heading.to,
      );
...
export const headingUpdateListener = EditorView.updateListener.of(
  (update: ViewUpdate) => {
    if (update.transactions.some((tr) => tr.annotation(isBetterHeadingUpdate)))
      return;
    ...
    const cursorHeading = headings.find(
      (h) => h.from <= cursorPos && cursorPos <= h.to,
    );
    ...
    if (cursorHeading) {
      newCursorPos =
        cursorHeading.from +
        cursorHeading.mdHeading.length +
        1 +
        cursorHeading.prefix.length +
        1;
    }
...
export const atomicPrefixRanges = EditorView.atomicRanges.of(
  (view: EditorView) => {
    ...
    const marks = headings.reduce(
      (accumulator: Range<Decoration>[], heading: BetterHeading) => {
        if (!heading.prefix) return accumulator;
        const prefixStart = heading.from;
        const prefixEnd =
          heading.from +
          heading.mdHeading.length +
          1 +
        ...
        if (prefixEnd <= heading.to + 1) {
          return produce(accumulator, (draftState) => {
            draftState.push(
              Decoration.mark({ inclusive: true }).range(
                prefixStart,
                prefixEnd,
              ),
            );
...
export const prefixDecorations = ViewPlugin.fromClass(
  class {
    ...
    private buildDecorations(state: EditorState): DecorationSet {
      ...
      const decorations = headings.reduce(
        (accumulator: Range<Decoration>[], heading: BetterHeading) => {
          if (!heading.prefix) return accumulator;
